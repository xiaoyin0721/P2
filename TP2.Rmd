...
\end{aligned}
$$
Dette er en antagelse at hvor der tjekkes om $\mu$ er ligmed $\mu_0$, som ofte vælges at sættes til 0, siden at dette ofte svarer til at det ingen effekt har, hvis ikke så forkastes $H_0$. Der opstilles også en konkurrence hypotese som er $H_a$, den alternativ hypotese. 
...
\end{aligned}
$$
Her hvor der tjekkes der igen om $\mu$ er ligmed $\mu_0$, hvis ikke så forkastes $H_0$ og $H_a$ accepteres. Dette vil så betyde $\mu$ har en effekt på populations parameter. Hvis en population havde $\mu$ = 10 og $\sigma$ = 2 og en stikprøve har $\overline{y}$ = 12, $s$ = 2 og n = 120, så ville denne stikprøve være signifikant. 
...
Der findes værdier som kan benyttes til at vurdere hvorvidt man beholder $H_0$ eller forkaster den. Disse vil forklares og arbejdes videre med i hypotesetest afsnittet. 
...
Et eksempel på at finde en $t_{crit}$ kunne være således:
```{r}
qdist("t", p = c(0.025, 0.975), df = 99)
```
$t_{crit}$ kan så aflæses til at være $\pm 1.984$. 
