...
We use Pearson's Chi-squared Test of Independence to test if  $U{i}$ og $U{i + 1}$ in PRNG generated sequences are independent of each other.Furthermore, the test result will be visualized by a spectral test.


```{r}

linear_congruence_heltal <- function(i, X_0) {
t <- 0
a <- 11102357
c <- 21353
m <- 2**32
...
while(t < i){
  X_0 <- (a*X_0+c)%%m
  v1 <- c(v1 , trunc(as.numeric(((X_0)/m))*10))
  t <- t + 1
}
return(v1)
}
obs <- linear_congruence_heltal(i=100, X_0 = 1029)
exp <- 10

sum((obs-exp)^2)/exp
#sum(power(obs-exp, 2)/e)

qchisq(0.95, 99, ncp = 0, lower.tail = TRUE)
