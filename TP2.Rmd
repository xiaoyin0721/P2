...
We use Pearson's Chi-squared Test of Independence to test if  $U{i}$ og $U{i + 1}$ in PRNG generated sequences are independent of each other.
Comparing observed frequencies is:
$$
\begin{aligned}
...
$o_i$ = Observed frequency for ith bin 
$e_i$ = Expected frequency

When $D$ score equals to zero, we will have an exact fit, D has a chi-square distribution with k-1 degrees of freedom. Comparing $D$ score with $\chi_[1-\alpha, k-1]$, if $D$ is less we can pass with confidence $\alpha$


```{r}

obs <- hist(linear_congruence(i=10000, X_0 = 1576))$counts
exp <-  10000/20
D_score <- sum((obs-exp)^2)/exp
Chi_significance <- qchisq(0.95, 19, ncp = 0) 
D_score 
Chi_significance
...
It is observed that the observed difference is much smaller than chi-squared value, therefore the LCG generated values follows a uniform distribution with 95% confidence. 
