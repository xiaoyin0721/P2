...
Sammenhængen mellem kartetiske og polære-koordinator er:
$x = cos(\theta)*r$
$y = sin(\theta)*r$
...
I en population findes der to værdier, $\mu$ og $\sigma$, som er interessante at kigge nærmere på. Middelværdien af en population kaldes $\mu$, imens populationens standardafvigelse kaldes $\sigma$.

Populationens standardafvigelse kan udregnes ud fra populationens varians. Nedenstående ligning viser, hvordan variansen for en stikprøve kan udregnes:
...
- Billede: 
En standardnormalfordeling er kendetegnet ved at $$\mu=0$$, og at $$\sigma = 1$$. Desuden har en standardnormalfordeling et klokkeformet udseende. Nedenfor er der plottet en standardnormalfordeling:
```{r}
qdist("norm",p=1,mean = 0, sd = 1,xlim = c(-4,4))
```
Der er en høj sandsynlighed for at få en værdi tæt på 0. Jo længere man afviger fra $$\mu$$, jo mindre er densiteten, som medfører at sandsynligheden for de givne værdier mindskes. Ergo vil sandsynligheden for at få 2, være mindre end sansynligheden for at få 0. 
En af anvendelserne for en standardnormalfordeling er, at den kan benyttes til en Z-test, som benyttes senere i projektet.
...
=======
En T-fordeling laves ud fra en standardnormalfordeling og dens udseende minder også om denne fordeling #Omformuler. Forskellen på en T-fordeling og en standardnormalfordeling, ligger i, at en T-fordeling laves ud fra frihedsgrader. For en stikprøve vil antallet af frihedsgrader være antallet af observationer minus 1, altså $n-1$. Jo flere frihedsgrader der er, jo mere ligner en T-fordeling en standardnormalfordeling. Nedenstående er der plottet 2 T-fordelinger med henholdsvis 5 og 100 frihedsgrader:
>>>>>>> a3f10b4ec4319f1bd4faf77a2205dd76e8a13d03
...
```
Der er altså en $10.4\%$ chance for at en begivenhed sker 7 gange, hvis begivenhedsraten er 5.

```{r}
xvalue <- 0:20
dpois(7, 5)
plot(xvalue, dpois(0:20, 5), type = "h")
```

### Uniform fordeling
En uniform fordeling, er hvor dataet bliver indelt i lige store intervaller, og alle intervaller, har lige antal observationer. #Lige sandsynlighed for udfald?
Det er vigtigt at notere at hvis antallet af observationer er lavt, vil fordelingen ikke tilsyneladende være uniform. Dette vises ved at lave to uniformfordelinger på det samme seed,hvor den eneste forskel er antallet af observationer. Dette kan ses i de nedenstående grafer. 
...
>>>>>>> a706e120c821c6f8e2813a1511540acf0be6a33a
...
### Hypoteser
En hypotese er et udsagn om en given population. Den er også angivet som en populations parameter som har en given værdi eller er indenfor et bestemt interval. Det gør det så muligt at beregne om en hypotese er sand eller falsk. For at kunne sørge for at man anvender en korrekt analyse metode på en stikprøve, så opsættes en hypotese omkring sammenhængen af ens data. Der kan så opstilles en hypotese test, hvilket er en metode hvor man sætter en hypotese mod en anden hypotese. Metoden sætter man den alternativ hypotese ($H_a$) mod en konkurrence hypotese som er nulhypotesen ($H_0$). En stikprøves vil så tages ud af en population, hvor $H_0$ testes imod $H_a$ ved brug af et signifikansniveau ($\alpha$) og en $p$-værdi. 
...
For at tjekke hvilken af hypoteserne er sand eller falsk, vælges et signifikansniveau som ofte er $\alpha$ = 5%. Ud fra dette signifikansniveau bliver der udregnet en kritisk værdi på en sandsynlighedsfordeling.
